...
def frame_f1(probs: torch.Tensor, gold: torch.Tensor, **kwargs) -> float:
    ...
    gold: [sequence_length]
    """
    return f1_score(gold.cpu().numpy(), probs.argmax(dim=1).cpu().numpy(), **kwargs, average='weighted')
...
def frame_precision(probs: torch.Tensor, gold: torch.Tensor, **kwargs) -> float:
    ...
    gold: [sequence_length]
    """
    return precision_score(gold.cpu().numpy(), probs.cpu().argmax(dim=1).numpy(), **kwargs)
...
def frame_recall(probs: torch.Tensor, gold: torch.Tensor, **kwargs) -> float:
    ...
    gold: [sequence_length]
    """
    return recall_score(gold.cpu().numpy(), probs.cpu().argmax(dim=1).numpy(), **kwargs)
...
def frame_roc_auc(probs: torch.Tensor, gold: torch.Tensor, **kwargs) -> float:
    ...
    gold: [sequence_length]
    """
    return roc_auc_score(gold.cpu().numpy(), np.exp(probs.cpu().numpy()), **kwargs)
